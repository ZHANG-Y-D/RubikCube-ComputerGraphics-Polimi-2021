...
OBJModel.prototype.lookBufferInfo = function(gl){
	console.log("总组数：%d", this.groups.length);
	for(var i = 0; i < this.groups.length; i++){
		gl.vertexAttribPointer()
	}
}
OBJModel.prototype.update = function(gl, angle, attribIndex, mtlIndex, u_Sampler){
...
function loadMTL(mtlLib){
	...
	//让他们都读同一个material
	request.open('GET', 'Assert/Cube.mtl', true);
...
Group.prototype.draw = function(gl, attribIndex, mtlIndex, u_Sampler){
	...
	// 为u_Sampler传值
	if(u_Sampler != 0 && this.material.texture != null){
